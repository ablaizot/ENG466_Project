...
public:
  ...
  uint64_t t_assigned_;         //time at which the event was assigned
...
public:
  ...
  void markDone(uint64_t clk) {
    ...
    FILE* f = fopen("../../tmp/task_completion_time.txt", "w");
    if (f) {
      fprintf(f, "%lld\n", t_done_ - t_assigned_);
      fclose(f);
    }
